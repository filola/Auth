...
import { ICreatorsRepository } from '@profile/creators.interface';
import { CreateCreatorInputDto } from './dto/CreateCreator.input.dto';
...
export class AuthService {
    constructor(
        ...
        @Inject(ICreatorsRepository) private creatorsRepository: ICreatorsRepository,
    ...
    async createCreator(body: CreateCreatorInputDto, user) {
        const { id: CreatorId } = await this.creatorsRepository.findCreatorByUserId(user);
        await this.creatorsRepository.createCreatorLanguage(body, CreatorId);
        await this.creatorsRepository.createCreatorHashTag(body, CreatorId);
        await this.creatorsRepository.createCreatorDate(body, CreatorId);
        const result = await this.authRepository.createCreator(body, user);

        if (!result) {
            throw new BadRequestException('알수없는 에러.');
        }
    ...
    async generateCode(body: CheckEmailDto, sendEmail) {
        ...
        if (isUser && isUser.isMeta) {
            return { success: true, code: 400, data: '페이스북으로 가입한 유저입니다' };
        }
        const randomNumber = this.random.makeRand6Num();
        await this.cacheRepository.set(email, randomNumber, 180);
        sendEmail(email, randomNumber);
    ...
    async getUserIfRefreshTokenMatches(refreshToken: string, userId: number): Promise<any> {
        ...
        if (refreshToken === userRefreshToken) {
            const newAccessToken = this.jwtGenerate(userId, 'accessToken');
